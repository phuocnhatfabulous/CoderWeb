export type FeatureContest = {
  id: string
  name: string
  descriptions: string[]
  image: string
}

export const featureContests: FeatureContest[] = [
  {
    id: "contest-1",
    name: 'Tech War 2022',
    descriptions: [
      '🎯 Là sân chơi để sinh viên thể hiện năng lực về lập trình, tạo môi trường thực hành để sinh viên rèn luyện trở thành những chuyên gia công nghệ, những lập trình viên có chất lượng.',
      '🎯 Dành cho sinh viên Việt Nam đang học tập tại các trường đại học, cao đẳng và học viện trên toàn quốc. Đăng ký dự thi theo đội gồm tối đa 03 thành viên (có thể là sinh viên ở cùng trường hoặc ở các trường khác nhau).',
      'Có một điều mà các đội chơi cũng như các bạn khán giả đang ngóng đợi đó là cơ cấu giải thưởng của cuộc thi năm nay. Và ngày hôm nay, BTC xin bật mí cơ cấu giải thưởng như sau:',
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒕𝒖𝒂̂̀𝒏 :",
      "Mỗi tuần có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 1.000.000 đồng \n + 02 giải nhì: 500.000 đồng \n + 03 giải ba: 300.000 đồng \n + 03 giải khuyến khích: 150,000 đồng ",
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒄𝒉𝒖𝒏𝒈 𝒌𝒆̂́𝒕 : ",
      "Vào vòng chung kết của cuộc thi, trị giá giải thưởng sẽ vô cùng hấp dẫn! \n Có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 4.000.000 đồng \n + 02 giải nhì: 2.000.000 đồng \n + 03 giải ba: 1.000.000 đồng \n + 03 giải khuyến khích: 500.000 đồng",
      "Giải thưởng quá hấp dẫn phải không các bạn? Vậy còn chờ gì nữa mà không đăng kí tham gia ngay nào!!"
    ],
    image: '/tech-war.png'
  },
  {
    id: "contest-2",
    name: 'DevOL IT UTC2',
    descriptions: [
      '🎯 Là sân chơi để sinh viên thể hiện năng lực về lập trình, tạo môi trường thực hành để sinh viên rèn luyện trở thành những chuyên gia công nghệ, những lập trình viên có chất lượng.',
      '🎯 Dành cho sinh viên Việt Nam đang học tập tại các trường đại học, cao đẳng và học viện trên toàn quốc. Đăng ký dự thi theo đội gồm tối đa 03 thành viên (có thể là sinh viên ở cùng trường hoặc ở các trường khác nhau).',
      'Có một điều mà các đội chơi cũng như các bạn khán giả đang ngóng đợi đó là cơ cấu giải thưởng của cuộc thi năm nay. Và ngày hôm nay, BTC xin bật mí cơ cấu giải thưởng như sau:',
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒕𝒖𝒂̂̀𝒏 :",
      "Mỗi tuần có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 1.000.000 đồng \n + 02 giải nhì: 500.000 đồng \n + 03 giải ba: 300.000 đồng \n + 03 giải khuyến khích: 150,000 đồng ",
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒄𝒉𝒖𝒏𝒈 𝒌𝒆̂́𝒕 : ",
      "Vào vòng chung kết của cuộc thi, trị giá giải thưởng sẽ vô cùng hấp dẫn! \n Có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 4.000.000 đồng \n + 02 giải nhì: 2.000.000 đồng \n + 03 giải ba: 1.000.000 đồng \n + 03 giải khuyến khích: 500.000 đồng",
      "Giải thưởng quá hấp dẫn phải không các bạn? Vậy còn chờ gì nữa mà không đăng kí tham gia ngay nào!!"
    ],
    image: '/devol.jpg'
  },
  {
    id: "contest-3",
    name: 'Tech War',
    descriptions: [
      '🎯 Là sân chơi để sinh viên thể hiện năng lực về lập trình, tạo môi trường thực hành để sinh viên rèn luyện trở thành những chuyên gia công nghệ, những lập trình viên có chất lượng.',
      '🎯 Dành cho sinh viên Việt Nam đang học tập tại các trường đại học, cao đẳng và học viện trên toàn quốc. Đăng ký dự thi theo đội gồm tối đa 03 thành viên (có thể là sinh viên ở cùng trường hoặc ở các trường khác nhau).',
      'Có một điều mà các đội chơi cũng như các bạn khán giả đang ngóng đợi đó là cơ cấu giải thưởng của cuộc thi năm nay. Và ngày hôm nay, BTC xin bật mí cơ cấu giải thưởng như sau:',
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒕𝒖𝒂̂̀𝒏 :",
      "Mỗi tuần có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 1.000.000 đồng \n + 02 giải nhì: 500.000 đồng \n + 03 giải ba: 300.000 đồng \n + 03 giải khuyến khích: 150,000 đồng ",
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒄𝒉𝒖𝒏𝒈 𝒌𝒆̂́𝒕 : ",
      "Vào vòng chung kết của cuộc thi, trị giá giải thưởng sẽ vô cùng hấp dẫn! \n Có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 4.000.000 đồng \n + 02 giải nhì: 2.000.000 đồng \n + 03 giải ba: 1.000.000 đồng \n + 03 giải khuyến khích: 500.000 đồng",
      "Giải thưởng quá hấp dẫn phải không các bạn? Vậy còn chờ gì nữa mà không đăng kí tham gia ngay nào!!"
    ],
    image: '/nckh.jpg'
  },
  {
    id: "contest-4",
    name: 'Giải pháp Công nghệ thông tin trong thời đại 4.0',
    descriptions: [
      '🎯 Là sân chơi để sinh viên thể hiện năng lực về lập trình, tạo môi trường thực hành để sinh viên rèn luyện trở thành những chuyên gia công nghệ, những lập trình viên có chất lượng.',
      '🎯 Dành cho sinh viên Việt Nam đang học tập tại các trường đại học, cao đẳng và học viện trên toàn quốc. Đăng ký dự thi theo đội gồm tối đa 03 thành viên (có thể là sinh viên ở cùng trường hoặc ở các trường khác nhau).',
      'Có một điều mà các đội chơi cũng như các bạn khán giả đang ngóng đợi đó là cơ cấu giải thưởng của cuộc thi năm nay. Và ngày hôm nay, BTC xin bật mí cơ cấu giải thưởng như sau:',
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒕𝒖𝒂̂̀𝒏 :",
      "Mỗi tuần có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 1.000.000 đồng \n + 02 giải nhì: 500.000 đồng \n + 03 giải ba: 300.000 đồng \n + 03 giải khuyến khích: 150,000 đồng ",
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒄𝒉𝒖𝒏𝒈 𝒌𝒆̂́𝒕 : ",
      "Vào vòng chung kết của cuộc thi, trị giá giải thưởng sẽ vô cùng hấp dẫn! \n Có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 4.000.000 đồng \n + 02 giải nhì: 2.000.000 đồng \n + 03 giải ba: 1.000.000 đồng \n + 03 giải khuyến khích: 500.000 đồng",
      "Giải thưởng quá hấp dẫn phải không các bạn? Vậy còn chờ gì nữa mà không đăng kí tham gia ngay nào!!"
    ],
    image: '/cntt40.jpg'
  },
  {
    id: "contest-5",
    name: 'OLYMPIC Tin học sinh viên Việt Nam lần thứ 31',
    descriptions: [
      '🎯 Là sân chơi để sinh viên thể hiện năng lực về lập trình, tạo môi trường thực hành để sinh viên rèn luyện trở thành những chuyên gia công nghệ, những lập trình viên có chất lượng.',
      '🎯 Dành cho sinh viên Việt Nam đang học tập tại các trường đại học, cao đẳng và học viện trên toàn quốc. Đăng ký dự thi theo đội gồm tối đa 03 thành viên (có thể là sinh viên ở cùng trường hoặc ở các trường khác nhau).',
      'Có một điều mà các đội chơi cũng như các bạn khán giả đang ngóng đợi đó là cơ cấu giải thưởng của cuộc thi năm nay. Và ngày hôm nay, BTC xin bật mí cơ cấu giải thưởng như sau:',
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒕𝒖𝒂̂̀𝒏 :",
      "Mỗi tuần có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 1.000.000 đồng \n + 02 giải nhì: 500.000 đồng \n + 03 giải ba: 300.000 đồng \n + 03 giải khuyến khích: 150,000 đồng ",
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒄𝒉𝒖𝒏𝒈 𝒌𝒆̂́𝒕 : ",
      "Vào vòng chung kết của cuộc thi, trị giá giải thưởng sẽ vô cùng hấp dẫn! \n Có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 4.000.000 đồng \n + 02 giải nhì: 2.000.000 đồng \n + 03 giải ba: 1.000.000 đồng \n + 03 giải khuyến khích: 500.000 đồng",
      "Giải thưởng quá hấp dẫn phải không các bạn? Vậy còn chờ gì nữa mà không đăng kí tham gia ngay nào!!"
    ],
    image: '/olympicTH.jpg'
  },
  {
    id: "contest-6",
    name: 'Chuyên đề Trí tuệ nhân tạo',
    descriptions: [
      '🎯 Là sân chơi để sinh viên thể hiện năng lực về lập trình, tạo môi trường thực hành để sinh viên rèn luyện trở thành những chuyên gia công nghệ, những lập trình viên có chất lượng.',
      '🎯 Dành cho sinh viên Việt Nam đang học tập tại các trường đại học, cao đẳng và học viện trên toàn quốc. Đăng ký dự thi theo đội gồm tối đa 03 thành viên (có thể là sinh viên ở cùng trường hoặc ở các trường khác nhau).',
      'Có một điều mà các đội chơi cũng như các bạn khán giả đang ngóng đợi đó là cơ cấu giải thưởng của cuộc thi năm nay. Và ngày hôm nay, BTC xin bật mí cơ cấu giải thưởng như sau:',
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒕𝒖𝒂̂̀𝒏 :",
      "Mỗi tuần có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 1.000.000 đồng \n + 02 giải nhì: 500.000 đồng \n + 03 giải ba: 300.000 đồng \n + 03 giải khuyến khích: 150,000 đồng ",
      "𝑮𝒊𝒂̉𝒊 𝒕𝒉𝒖̛𝒐̛̉𝒏𝒈 𝒄𝒉𝒖𝒏𝒈 𝒌𝒆̂́𝒕 : ",
      "Vào vòng chung kết của cuộc thi, trị giá giải thưởng sẽ vô cùng hấp dẫn! \n Có 9 giải thưởng dành cho các thí sinh đạt giải nhất, nhì, ba và khuyến khích. ",
      "+ 01 giải nhất: 4.000.000 đồng \n + 02 giải nhì: 2.000.000 đồng \n + 03 giải ba: 1.000.000 đồng \n + 03 giải khuyến khích: 500.000 đồng",
      "Giải thưởng quá hấp dẫn phải không các bạn? Vậy còn chờ gì nữa mà không đăng kí tham gia ngay nào!!"
    ],
    image: '/ai.jpg'
  },
]